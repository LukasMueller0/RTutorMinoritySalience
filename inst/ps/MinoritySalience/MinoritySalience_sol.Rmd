...
# This function creates a skeleton for your problem set package
  rtutor.package.skel(sol.file=sol.file, ps.name=ps.name,libs=libs,
    pkg.name="RTutorMinoritySalience",   # Name of the problem set package
    pkg.parent.dir = "D:/RTutorProblemSet/RTutorMinoritySalience", # Parent directory 
    author="Lukas Mueller", # Your name
    github.user="LukasMueller0",     # Your github user name
...
question: Well done! Let's continue the analysis with a little quiz. What is the total number of variables within the data set?
answer: 15
roundto: 1
#>
...
#< quiz "Data type"
question: Which data type fits best with the data set NRW?
sc: 
    - Time Series Data
    - Cross-Sectional Data
    - Panel Data*
...
#< quiz "Group size"
question: Looking at the table, do you think the data set contains more observations of municipalities with a mosque or without a mosque?
sc: 
    - The data set contains the same number of observations of municipalities with a mosque and without a mosque.
    - The number of observations of municipalities with a mosque is larger.
    - The number of observations of municipalities without a mosque is larger.*
success: Good job!
failure: Try again.
...
#< quiz "Event-Study"
question: Which functions do you think should be used, and in what order?
sc: 
...
Google_event <- Google %>%
  filter(start >= -4 & start <= 4) %>%
  group_by(start, mosques) %>%
  summarize(mean = mean(searches_muslimindex, na.rm = TRUE)) %>%
  mutate(mosques = as.factor(mosques))
```
...
#< quiz "Ramadan"
question: Considering the previous exercise and the 15 variables included in our NRW data set, do you think these variables are sufficient to implement the model from above?
sc: 
    - Yes
    - No*
success: Good job!
...
#< quiz "Ramadan Start"
question: When did Ramadan start this year? Please enter the answer in the following example format 22.03.2023.
answer: 10.03.2024
roundto: 1
#>
...
reg_right <- lm(right ~ ramadan, data = NRW)
reg_left <- lm(left ~ ramadan, data = NRW)
reg_establish <- lm(establish ~ ramadan, data = NRW)
reg_turnout <- lm(turnout ~ ramadan, data = NRW)

stargazer(reg_right, reg_left, reg_establish, reg_turnout, align = TRUE, dep.var.labels = c("Right", "Left", "Established", "Turnout"), covariate.labels = c("Ramadan"), type = "html")
```

#< quiz "Causality"
question: Which interpretation of the model is accurate?
mc: 
    - Ramadan results in an increased voting share for all parties and an increased voter turnout.
    - Ramadan increases the voting share of right-wing parties by 0.769 percentage points.
    - During Ramadan we can observe an increase in the voting share across all political parties and in voter turnout.*
success: Good job!
failure: Try again.
...
#< quiz "Mosque Proxy"
question: Did we observe differing Google search activity in municipalities with a mosque compared to those without one?
sc: 
    - Yes*
    - No
success: Good job!
...
DiD_data_pre <- NRW %>%
  filter(ramadan == 0) %>%
  group_by(dummy_mosque) %>%
  summarise(mean = mean(right, na.rm = TRUE))
```
...
DiD_data <- NRW %>%
  group_by(dummy_mosque,ramadan) %>%
  summarize(mean = mean(right, na.rm = TRUE))
```
...
#< quiz "DiD Formula"
question: Pick the correct formula.
sc: 
    - 1*
    - 2 
success: Good job!
...
DiD <- (treatment_post-control_post)-(treatment_pre-control_pre)
DiD
...
#< quiz "DiD Estimator"
question: Based on the formula provided above, which coefficient represents the DiD estimator and thus captures the causal effect?
sc: 
    - \(\beta_1\)
    - \(\beta_2\)
    - \(\beta_3\)*
...
#< quiz "Beta 1"
question: How can we interpret $\textit{ß}_{\textit{1}}$?
sc: 
    - Control group difference between the pre and post treatment phase 
    - Treatment group difference between the pre and post treatment phase
    - The Difference between the treatment and control group in the pre treatment phase*
success: Good job!
failure: Try again.
...
#< quiz "Beta 2"
question: How can we interpret $\textit{ß}_{\textit{2}}$?
sc: 
    - Control group difference between the pre and post treatment phase*
    - Treatment group difference between the pre and post treatment phase
    - The Difference between the treatment and control group in the pre treatment phase
...
reg_right <- felm(right ~ dummy_mosque + ramadan + dummy_mosque*ramadan, data = NRW)
reg_left <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan, data = NRW)
reg_established <- felm(establish ~ dummy_mosque + ramadan + dummy_mosque*ramadan, data = NRW)
reg_turnout <- felm(turnout ~ dummy_mosque + ramadan + dummy_mosque*ramadan, data = NRW)
...
#< quiz "Constant"
question: How do you think the constant term should be interpreted appropriately?
sc: 
    - The constant represents the average treatment effect for the entire sample.
    - The constant represents the baseline level of the dependent variable for the control group during the pre-treatment period.*
    - The constant shows how much the treatment period affects the dependent variable.
success: Good job!
failure: Try again.
...
#< quiz "Fixed Effects"
question: Do you think adding the district_name, which serves as a unique ID for each municipality in our data set, would be a good candidate for a Unit-Varying Fixed Effect?
sc: 
    - Yes*
    - No
success: Good job!
...
reg_original <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan, data = NRW)
reg_left <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan | district_name, data = NRW)
...
reg_original <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan | district_name, data = NRW)
reg_control <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan + election_type | district_name, data = NRW)
reg_fe <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan | district_name + election_type, data = NRW)
...
reg_ind <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan | district_name + election_type, data = NRW)
reg_comb <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan | district_name:election_type, data = NRW)
...
reg_unit <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan | district_name:election_type, data = NRW)
reg_unittime <- felm(left ~ dummy_mosque + ramadan + dummy_mosque*ramadan | district_name:election_type + date_election, data = NRW)
...
#< quiz "Where is Ramadan gone?"
question: What do you think might be the reason for this phenomenon?
sc: 
    - The variable Ramadan has no influence on the election outcomes
    - The election date accounts for the variation that Ramadan would normally explain*
success: Exactly!
failure: Try again.
...
share_foreign_minmax <- NRW %>%
  ...
  summarize(Minimum = min(share_foreign, na.rm = TRUE),
            Maximum = max(share_foreign, na.rm = TRUE),
            SD = sd(share_foreign, na.rm = TRUE)) %>%
  arrange(desc(SD))
...
NRW %>% filter(district_name == 318) %>%
  ggplot(aes(x=year, y=share_foreign)) +
  geom_line() +
  labs(x="Years", y="Share of foreign citzizens in %") +
  ggtitle("Change of share_foreign in District 318") +
  theme_minimal() +
...
#< quiz "Fixed Effects not fix?"
question: Now, if you think back and recall the explanation of unit-varying fixed effects, what do you think could be a potential problem with our model?
sc: 
    - The fixed effects model assumes that the foreigner quota is constant over time within each district, but in reality, it changes, which could lead to biased estimators*
    - The fixed effects model assumes that the foreigner quota is the same across all districts, which could lead to incorrect conclusions.
success: Exactly!
failure: Try again.
...
#< quiz "Definition of Ramadan"
question: Do you recall how we defined the Ramadan variable in the previous chapters?
sc: 
    - Ramadan is 1 if the election took part 45 days after the beginning of Ramadan.
    - Ramadan is 1 if the election took part 90 days after the beginning of Ramadan.*
    - Ramadan is 1 if the election took part 135 days after the beginning of Ramadan.
success: Exactly!
failure: Try again.
...
#< quiz "Ramadan Trends"
question: If you think about it, what trend would you expect when varying the time windows?
sc: 
    - The effect of minority salience remains constant and is unaffected by different time windows.
    - The impact on voting behavior is likely strongest immediately following Ramadan.*
    - As more time passes after Ramadan, the effect on voting behavior may intensify further.
success: Correct!
failure: Try again.
...
NRW <- NRW %>%
  mutate(ramadan45 = ifelse(distance_ramadan <=45,1,0),
         ramadan75 = ifelse(distance_ramadan <=75,1,0),
         ramadan90 = ifelse(distance_ramadan <=90,1,0),
         ramadan120 = ifelse(distance_ramadan <=120,1,0),
         ramadan160 = ifelse(distance_ramadan <=160,1,0)
  )
```
...
question: What do you think happens if we increase the confidence level of our interval?
sc: 
    - The confidence interval gets wider.*
    - The confidence interval gets more narrow.
success: Correct!
failure: Try again.
...
result %>%
  ggplot(aes(x = DaysSinceRamadan, y = Estimate)) +
  geom_point() +
  scale_x_continuous(breaks = c(-30,0,45,75,90,120,160)) +
  geom_errorbar(aes(x = DaysSinceRamadan, ymin = lower005, ymax = upper005), color = "grey", width = 0, size = 0.75) +
  theme_classic() +
...
Twitter <- read_dta("NRW_twitter.dta")
...
Attacks <- Attacks %>%
  mutate(ramadan = ifelse(daysince <= 90 & daysince >= 0,1,0),
         month = month(date), 
         year = year(date),
         day = wday(date),
         day_of_year = yday(date))
...
attacks_reg <- felm(n_attacks ~ ramadan | as.factor(day) + as.factor(day_of_year) + as.factor(month):as.factor(year), data = Attacks)
stargazer(attacks_reg, align = TRUE, dep.var.labels = c("Attacks"), covariate.labels = c("Ramadan"), type = "html")
```
